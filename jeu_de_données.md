...
CREATE TABLE loans (
    ...
    CONSTRAINT fk_loans_user FOREIGN KEY (id_user) REFERENCES users(id_user),
    CONSTRAINT fk_loans_book FOREIGN KEY (id_book) REFERENCES books(id_book)
);
GO
...
CREATE TABLE reservations (
    ...
    CONSTRAINT fk_reservations_user FOREIGN KEY (id_user) REFERENCES users(id_user),
    CONSTRAINT fk_reservations_book FOREIGN KEY (id_book) REFERENCES books(id_book)
);
GO
...
CREATE TABLE ratings (
    ...
    CONSTRAINT fk_ratings_user FOREIGN KEY (id_user) REFERENCES users(id_user),
    CONSTRAINT fk_ratings_book FOREIGN KEY (id_book) REFERENCES books(id_book)
);
GO
...
CREATE TABLE comments (
    ...
    CONSTRAINT fk_comments_book FOREIGN KEY (id_book) REFERENCES books(id_book)
);
GO
...
CREATE TABLE favorites (
    ...
    CONSTRAINT fk_favorites_book FOREIGN KEY (id_book) REFERENCES books(id_book)
);
GO

--    Mot de passe pour tous : "password"

INSERT INTO users (email, password, first_name, last_name, phone, role, inscription_date, active, tos_acceptation_date)
VALUES
    ('bibliothecaire@bookhub.fr',
     '{bcrypt}$2a$10$KQHvqgijKFG3joJZMu7NXezoqyfAeFO96QLy2Cne4vCD3weqgTkeq',
     'Marie', 'Dupont', '0612345678', 'LIBRARIAN', GETDATE(), 1, GETDATE()),

    ('alice@bookhub.fr',
     '{bcrypt}$2a$10$KQHvqgijKFG3joJZMu7NXezoqyfAeFO96QLy2Cne4vCD3weqgTkeq',
     'Alice', 'Martin', '0698765432', 'USER', GETDATE(), 1, GETDATE()),

    ('bob@bookhub.fr',
     '{bcrypt}$2a$10$KQHvqgijKFG3joJZMu7NXezoqyfAeFO96QLy2Cne4vCD3weqgTkeq',
     'Bob', 'Leroy', '0678901234', 'USER', GETDATE(), 1, GETDATE());
GO


INSERT INTO books (title, author, isbn, description, category, cover_url, total_copies, available_copies, added_at, avg_rating)
VALUES
    -- Fiction
    ('Les Ombres du Temps',
     'Claire Fontaine',
...
INSERT INTO ratings (id_user, id_book, note, rating_date)
VALUES
    -- Alice note plusieurs livres
    ((SELECT id_user FROM users WHERE email = 'alice@bookhub.fr'),
     (SELECT id_book FROM books WHERE isbn = '978-2-1234-0001-1'), 4, DATEADD(day, -20, GETDATE())),

    ((SELECT id_user FROM users WHERE email = 'alice@bookhub.fr'),
     (SELECT id_book FROM books WHERE isbn = '978-2-1234-0002-2'), 5, DATEADD(day, -18, GETDATE())),

    ((SELECT id_user FROM users WHERE email = 'alice@bookhub.fr'),
     (SELECT id_book FROM books WHERE isbn = '978-2-1234-0004-4'), 5, DATEADD(day, -15, GETDATE())),

    ((SELECT id_user FROM users WHERE email = 'alice@bookhub.fr'),
     (SELECT id_book FROM books WHERE isbn = '978-2-1234-0005-5'), 4, DATEADD(day, -10, GETDATE())),

    ((SELECT id_user FROM users WHERE email = 'alice@bookhub.fr'),
     (SELECT id_book FROM books WHERE isbn = '978-2-1234-0008-8'), 4, DATEADD(day, -8, GETDATE())),

    -- Bob note plusieurs livres
    ((SELECT id_user FROM users WHERE email = 'bob@bookhub.fr'),
     (SELECT id_book FROM books WHERE isbn = '978-2-1234-0001-1'), 5, DATEADD(day, -19, GETDATE())),

    ((SELECT id_user FROM users WHERE email = 'bob@bookhub.fr'),
     (SELECT id_book FROM books WHERE isbn = '978-2-1234-0003-3'), 4, DATEADD(day, -17, GETDATE())),

    ((SELECT id_user FROM users WHERE email = 'bob@bookhub.fr'),
     (SELECT id_book FROM books WHERE isbn = '978-2-1234-0007-7'), 5, DATEADD(day, -14, GETDATE())),

    ((SELECT id_user FROM users WHERE email = 'bob@bookhub.fr'),
     (SELECT id_book FROM books WHERE isbn = '978-2-1234-0011-1'), 5, DATEADD(day, -9, GETDATE())),

    ((SELECT id_user FROM users WHERE email = 'bob@bookhub.fr'),
     (SELECT id_book FROM books WHERE isbn = '978-2-1234-0016-6'), 5, DATEADD(day, -6, GETDATE())),

    -- Admin note
    ((SELECT id_user FROM users WHERE email = 'admin@bookhub.fr'),
     (SELECT id_book FROM books WHERE isbn = '978-2-1234-0002-2'), 4, DATEADD(day, -12, GETDATE())),

    ((SELECT id_user FROM users WHERE email = 'admin@bookhub.fr'),
     (SELECT id_book FROM books WHERE isbn = '978-2-1234-0017-7'), 5, DATEADD(day, -5, GETDATE()));
GO

-- Mise à jour des notes moyennes calculées
UPDATE books SET avg_rating = 4.50
WHERE isbn = '978-2-1234-0001-1'; -- (4+5)/2

UPDATE books SET avg_rating = 4.50
WHERE isbn = '978-2-1234-0002-2'; -- (5+4)/2

UPDATE books SET avg_rating = 4.00
WHERE isbn = '978-2-1234-0007-7'; -- (5)/1 = 5.00
-- (les avg_rating des autres livres sont déjà corrects depuis l'INSERT initial)
GO


INSERT INTO comments (id_user, id_book, comment, reported, comment_date)
VALUES
    ((SELECT id_user FROM users WHERE email = 'alice@bookhub.fr'),
     (SELECT id_book FROM books WHERE isbn = '978-2-1234-0001-1'),
    ...
    ((SELECT id_user FROM users WHERE email = 'alice@bookhub.fr'),
     (SELECT id_book FROM books WHERE isbn = '978-2-1234-0002-2'),
    ...
    ((SELECT id_user FROM users WHERE email = 'bob@bookhub.fr'),
     (SELECT id_book FROM books WHERE isbn = '978-2-1234-0016-6'),
     'Un travail de mémoire remarquable. Ces femmes méritaient d''être connues. À lire absolument.',
     0, DATEADD(day, -6, GETDATE())),

    ((SELECT id_user FROM users WHERE email = 'admin@bookhub.fr'),
     (SELECT id_book FROM books WHERE isbn = '978-2-1234-0017-7'),
     'Une épopée fantastique de très grande qualité. On sent le soin apporté à la construction du monde.',
     0, DATEADD(day, -5, GETDATE()));
GO


INSERT INTO loans (id_user, id_book, loan_date, due_date, return_date, status)
VALUES
    -- Alice emprunte "Le Dernier Témoin" (isbn 0003, available=0)
    ((SELECT id_user FROM users WHERE email = 'alice@bookhub.fr'),
     (SELECT id_book FROM books WHERE isbn = '978-2-1234-0003-3'),
     DATEADD(day, -10, GETDATE()), DATEADD(day, 4, GETDATE()), NULL, 'ACTIVE'),

    -- Bob emprunte "Signal Perdu" (isbn 0012, available=0)
    ((SELECT id_user FROM users WHERE email = 'bob@bookhub.fr'),
     (SELECT id_book FROM books WHERE isbn = '978-2-1234-0012-2'),
     DATEADD(day, -7, GETDATE()), DATEADD(day, 7, GETDATE()), NULL, 'ACTIVE'),

    -- Emprunt terminé (pour historique)
    ((SELECT id_user FROM users WHERE email = 'alice@bookhub.fr'),
     (SELECT id_book FROM books WHERE isbn = '978-2-1234-0005-5'),
    ...
    ((SELECT id_user FROM users WHERE email = 'bob@bookhub.fr'),
     (SELECT id_book FROM books WHERE isbn = '978-2-1234-0019-9'),
     DATEADD(day, -20, GETDATE()), DATEADD(day, -6, GETDATE()), NULL, 'OVERDUE');
GO



INSERT INTO reservations (id_user, id_book, reservation_date, rank_in_line, status)
VALUES
    -- Alice réserve "Signal Perdu" (déjà emprunté par Bob)
    ((SELECT id_user FROM users WHERE email = 'alice@bookhub.fr'),
     (SELECT id_book FROM books WHERE isbn = '978-2-1234-0012-2'),
...
INSERT INTO favorites (id_user, id_book)
VALUES
    ((SELECT id_user FROM users WHERE email = 'alice@bookhub.fr'),
     (SELECT id_book FROM books WHERE isbn = '978-2-1234-0002-2')),
